...
function Notes() {
  ...
  useEffect(() => {
    if (selectedCampaign) {
      const fetchCharactersAndPlaces = async () => {
        ...
        setCharacters(allCharacters.filter(character => character.campaign === selectedCampaign));
        setPlaces(allPlaces.filter(place => place.campaign === selectedCampaign));
      };
  ...
  const handlePlaceSelect = (e) => {
    const selectedPlaces = Array.from(e.target.selectedOptions, option => option.value);
    setNewNote({
      ...newNote,
      ...
      <ul>
        {notes.map(note => (
          <li key={note._id}>
            <h4>{note.name}</h4>
            <p>{note.content}</p>
            <p><strong>Campaign:</strong> {note.campaign.name}</p>
            <p><strong>Characters:</strong> {note.characters.map(character => character.name).join(', ')}</p>
            <p><strong>Places:</strong> {note.places.map(place => place.name).join(', ')}</p>
          </li>
        ))}
      </ul>
    </div>
